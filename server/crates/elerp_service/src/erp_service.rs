...
use super::{model::{authenticated_user::AuthenticatedUser, clear_result::ClearResult}, AppState};
...
async fn clear_persons(
    ...
    tx.commit().await?;
    s.ps.notice(WebSocketFlags::ClearPersons).await?;
    Ok(Json(ClearResult {
        success,
        failed,
    }))
...
async fn clear_areas(
    ...
    tx.commit().await?;
    s.ps.notice(WebSocketFlags::ClearAreas).await?;
    Ok(Json(ClearResult {
        success,
        failed,
    }))
...
async fn clear_warehouses(
    ...
    tx.commit().await?;
    s.ps.notice(WebSocketFlags::ClearWarehouses).await?;
    Ok(Json(ClearResult {
        success,
        failed,
    }))
...
async fn clear_order_categories(
    ...
    tx.commit().await?;
    s.ps.notice(WebSocketFlags::ClearOrderCategories).await?;
    Ok(Json(ClearResult {
        success,
        failed,
    }))
...
async fn clear_order_payments(
    ...
    tx.commit().await?;
    s.ps.notice(WebSocketFlags::ClearOrderPayments).await?;
    Ok(Json(ClearResult {
        success,
        failed,
    }))
...
async fn confirm_guest_order(
    ...
    if !s.erp.guest_order.is_exists(id, tx.as_mut()).await? {
        return AppError::custom(CustomErrorCode::OrderNotFound, "Guest order is not found.").into_err();
    }
   
    let token = match headers.get("X-Sub-Authorization") {
    Some(client_token) => {
        let token = client_token.to_str().unwrap_or("");
        if !s.erp.guest_order.is_sub_token_same(id, token, tx.as_mut()).await? {
    ...
    if s.us.get_sub_token_owner(token, tx.as_mut()).await?.is_none() {
        return AppError::custom(CustomErrorCode::UserNotFound, "Can't found the owner of your token!").into_err();
    }
    match s.erp.guest_order.get_status(id, tx.as_mut()).await? {
...
match s.erp.guest_order.confirm(id, body, tx.as_mut()).await? {
 Some(result) => Ok(Json(result)),
 None => AppError::custom(CustomErrorCode::OrderNotFound, "Order is not found.").into_err()
}
}
...
async fn clear_orders(
    ...
    tx.commit().await?;
    s.ps.notice(WebSocketFlags::ClearOrders).await?;
    Ok(Json(ClearResult {
        success,
        failed,
    }))
...
async fn clear_skus(
    ...
    tx.commit().await?;
    s.ps.notice(WebSocketFlags::ClearSKUs).await?;
    Ok(Json(ClearResult {
        success,
        failed,
    }))
...
async fn clear_sku_categories(
    ...
    tx.commit().await?;
    s.ps.notice(WebSocketFlags::ClearSKUCategories).await?;
    Ok(Json(ClearResult {
        success,
        failed,
    }))
