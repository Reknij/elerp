...
pub async fn update(config: AppConfig) -> bool {
    PublicSystem::update(config).await
}

pub async fn serve(config: AppConfig) {
    if PublicSystem::update(config.clone()).await {
        info!("Updated...");
    }
    info!("Using {:#?}", config);
    ...
    let api_router = Router::new().nest("/erp", erp_service::get_services()).nest("/us", user_service::get_services());
    let serve_dir = ServeDir::new(&config.web.dist).not_found_service(ServeFile::new(&config.web.dist.join(&config.web.index)));
    let app = Router::new()
        .nest_service("/", serve_dir)
        .merge(RapiDoc::with_openapi("/crm-api-docs/openapi.json", erp_openapi).path("/rapidoc/erp"))
    ...
    let enable_tls = {
        if config.tls.self_tls {
            ...
            Some((cert.serialize_pem().unwrap().as_bytes().to_vec(), cert.serialize_private_key_pem().as_bytes().to_vec()))
            ...
            Some((
                ...
                fs::read(key_path).await.expect("Can't read the private key path"),
            ))
        } else {
            None
        }
    ...
    if let Some((cert, key)) = enable_tls {
        ...
        axum_server::bind_rustls(format!("{}:{}", config.host, config.port).parse().unwrap(), rustls_config)
