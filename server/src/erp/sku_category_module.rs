...
use self::model::{GetSKUCategoriesQuery, SKUCategory};
pub mod model;
use super::Result;
...
impl SKUCategoryModule {
    ...
    pub async fn is_exists(&self, id: i64, tx: &mut SqliteConnection) -> Result<bool> {
        self.ps
            .is_exists_in_table("sku_categories", "id", id, tx)
            .await
    }

    pub async fn is_limit_reached(&self, tx: &mut SqliteConnection) -> Result<bool> {
        let count: i64 = sqlx::query("SELECT COUNT(*) as count FROM sku_categories;")
            .fetch_one(&mut *tx)
            .await?
            .get("count");
    ...
    pub async fn is_exists_name(
        ...
        self.ps
            .row_is_duplicate_col_in_table(name, prev, "sku_categories", "name", tx)
            .await
    }

    pub async fn add(&self, mut v: SKUCategory, tx: &mut SqliteConnection) -> Result<SKUCategory> {
        let r = sqlx::query(
            "INSERT INTO sku_categories (name, description, color, text_color) VALUES(?, ?, ?, ?)",
        )
        .bind(&v.name)
        .bind(&v.description)
        .bind(&v.color)
        ...
        v.id = self
            .ps
            .try_set_standard_id(r.last_insert_rowid(), "sku_categories", tx)
            .await?;
    ...
    pub async fn remove(&self, id: i64, notice: bool, tx: &mut SqliteConnection) -> Result<bool> {
        let r = self
            .ps
            .remove_row_from_table(id, "sku_categories", tx)
            .await?;
        if notice {
            self.ps
                .notice(WebSocketFlags::RemoveSKUCategory(id))
                .await?;
        }
        Ok(r)
    }
    ...
    pub async fn get(&self, id: i64, tx: &mut SqliteConnection) -> Result<Option<SKUCategory>> {
        self.ps
            .get_row_from_table("sku_categories", "id", id, tx)
            .await
    }
    ...
    pub async fn get_multiple(
        ...
        query: &GetSKUCategoriesQuery,
        tx: &mut SqliteConnection,
    ) -> Result<Vec<SKUCategory>> {
    ...
    pub async fn get_count(
        ...
        let row = sqlx::query(&format!(
            "SELECT count(*) as count FROM sku_categories {qw}"
        ))
        .fetch_one(&mut *tx)
        .await?;
        Ok(row.get("count"))
    ...
    pub async fn update(
        ...
        mut v: SKUCategory,
        tx: &mut SqliteConnection,
    ) -> Result<Option<SKUCategory>> {
        let r = sqlx::query(
            "UPDATE sku_categories SET name=?, description=?, color=?, text_color=? WHERE id=?",
        )
        .bind(&v.name)
        .bind(&v.description)
        .bind(&v.color)
        ...
        Ok(if r.rows_affected() == 1 {
            v.id = id;
            self.ps
                .notice(WebSocketFlags::UpdateSKUCategory(v.id))
                .await?;
            Some(v)
        } else {
            None
    ...
    pub async fn is_depend_by_another(&self, id: i64, tx: &mut SqliteConnection) -> Result<bool> {
        Ok(
            sqlx::query("SELECT id FROM sku_list WHERE sku_category_id=?")
                .bind(id)
                .fetch(&mut *tx)
                .try_next()
                .await?
