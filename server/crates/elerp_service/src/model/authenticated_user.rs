...
use elerp_common::{
    ...
    user_system::model::{
        ...
        user_permission::{
            ADD_ORDER, ADD_ORDER_PAYMENT, MANAGE_AREA, MANAGE_ORDER_CATEGORY, MANAGE_PERSON, MANAGE_SKU, MANAGE_SKU_CATEGORY, MANAGE_WAREHOUSE, UPDATE_REMOVE_ORDER, UPDATE_REMOVE_ORDER_PAYMENT
        },
    },
};
    ...
    S: Send + Sync,
{
    ...
    async fn from_request_parts(parts: &mut Parts, s: &S) -> Result<Self, Self::Rejection> {
        ...
        if let Some(auth) = auth {
            ...
            let user = app
                .us
                .token_to_user(auth, ActionType::System, tx.as_mut())
                .await?;
