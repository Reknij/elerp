...
use self::model::{
    fn_argument::{UserInfoID, WarehouseIsFrom},
    warehouse::GetWarehousesQuery,
};

use super::{ActionType, Result};
use anyhow::bail;
use futures::TryStreamExt;
use model::warehouse::Warehouse;
...
impl WarehouseModule {
    ...
    pub async fn is_limit_reached(&self, tx: &mut SqliteConnection) -> Result<bool> {
        let count: i64 = sqlx::query("SELECT COUNT(*) as count FROM warehouses;")
            .fetch_one(&mut *tx)
            .await?
            .get("count");
    ...
    pub async fn is_exists_name(
        ...
        self.ps
            .row_is_duplicate_col_in_table(name, prev, "warehouses", "name", tx)
            .await
    }
    ...
    pub async fn add(
        ...
        let r = sqlx::query(
            "INSERT INTO warehouses (name, description, address, person_in_charge_id, area_id, color, text_color) VALUES(?, ?, ?, ?, ?, ?, ?)",
        )
        .bind(&warehouse.name)
        ...
        warehouse.id = self
            .ps
            .try_set_standard_id(r.last_insert_rowid(), "warehouses", tx)
            .await?;
        self.ps
            .notice(WebSocketFlags::AddWarehouse(warehouse.id))
            .await?;
        Ok(warehouse)
    }
    ...
    pub async fn remove(&self, warehouse_id: i64, notice: bool, tx: &mut SqliteConnection) -> Result<bool> {
        let r = self
            .ps
            .remove_row_from_table(warehouse_id, "warehouses", tx)
            .await?;
        if notice {
            self.ps
            .notice(WebSocketFlags::RemoveWarehouse(warehouse_id))
            .await?;
        }
        Ok(r)
    }
    ...
    pub async fn get(&self, id: i64, tx: &mut SqliteConnection) -> Result<Option<Warehouse>> {
        self.ps.get_row_from_table("warehouses", "id", id, tx).await
    }

    pub async fn is_exists(&self, id: i64, tx: &mut SqliteConnection) -> Result<bool> {
        self.ps.is_exists_in_table("warehouses", "id", id, tx).await
    }
    ...
    pub async fn get_count(
        ...
        let row = sqlx::query(&format!(
            "SELECT count(*) as count FROM warehouses {inner} {qw}"
        ))
        .fetch_one(&mut *tx)
        .await?;
        Ok(row.get("count"))
    ...
    pub async fn update(
        ...
        let r = sqlx::query(
            "UPDATE warehouses SET name=?, description=?, person_in_charge_id=?, area_id=?, address=?, color=?, text_color=? WHERE id=?",
        )
        .bind(&v.name)
        .bind(&v.description)
        ...
        Ok(if r.rows_affected() == 1 {
            v.id = id;
            self.ps
                .notice(WebSocketFlags::UpdateWarehouse(v.id))
                .await?;
            Some(v)
        } else {
            None
    ...
    pub async fn is_depend_by_another(&self, id: i64, tx: &mut SqliteConnection) -> Result<bool> {
        Ok(sqlx::query("SELECT id FROM orders WHERE warehouse_id=?")
            .bind(id)
            .fetch(&mut *tx)
            .try_next()
            .await?
    ...
    pub async fn link(
        ...
        if is_link {
            self.ps
                .notice(WebSocketFlags::LinkedWarehouse(warehouse_id))
                .await?;
    ...
    pub async fn unlink(
        ...
        let is_unlink =
            sqlx::query("DELETE FROM warehouse_permission WHERE warehouse_id=? AND user_id=?;")
                .bind(warehouse_id)
                .bind(user_id)
                .execute(&mut *tx)
                .await?
        ...
        if is_unlink {
            self.ps
                .notice(WebSocketFlags::UnlinkedWarehouse(warehouse_id))
                .await?;
            self.ps
                .notice(WebSocketFlags::UnlinkedUser(user_id))
                .await?;
        }
        Ok(is_unlink)
    }

    pub async fn is_linked<'a>(
        &self,
        warehouse: WarehouseIsFrom,
        user: UserInfoID<'a>,
        tx: &mut SqliteConnection,
    ) -> Result<bool> {
        ...
        let query = match warehouse {
            WarehouseIsFrom::ID(warehouse_id) => sqlx::query(
                "SELECT user_id FROM warehouse_permission WHERE warehouse_id=? AND user_id=?;",
            )
            .bind(warehouse_id)
            .bind(user_id),
    ...
    #[allow(dead_code)]
    pub async fn get_linked_warehouses(
        &self,
        user_id: i64,
        pagination: &Pagination,
        tx: &mut SqliteConnection,
